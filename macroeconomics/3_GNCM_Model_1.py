...
tau_bar_eff = gamma * (
    (phi_L / (1 - beta_hat) + (1 - phi_L) * phi_0 / (1 - beta_hat * (1 - phi)))
)
...
@jit(nopython=True)
def eta_x(K, L, X_e):
    return ((1 - kappa) * X_e**rho) / (
        kappa * G(K, L) ** rho + (1 - kappa) * X_e**rho
    )
...
def compute_X(K, N, v_old, S_1_old, S_2_old, h, e):
    Phi = (
        lambda X: eta_x(K, N * h, e * X) * F(K, N * h, e * X) / X
...
S_2_in = (
    S_target_2005 - S_target_2015 + X_target * (phi_L + phi_0 * (1 - phi_L))
) / phi
...
val = (
    (Y_target / (1 - D_0_bar)) ** rho - (1 - kappa) * (e_0 * X_target) ** rho
) / kappa
    ...
    for m in range(T_horizon + 1):
        ...
        while finished == 0 and counter <= max_iter:
            ...
            for t in range(m, m + T_ahead):
                ...
                # h = (1+g_h)*h
                # e = (1+g_e)*e
                tau_in_dollar_per_ton_of_CO2_series[t, p] = (
                    tau_bar * Y * 10**6 * 12 / 44
                )
...
if show_plots:
    ...
    plt.plot(
        Time_window[0:T_plot],
        Double_pre_industrial[0:T_plot],
        color="red",
        linestyle="--",
        label="Double pre-industrial",
    )
    plt.xlabel("t (decades)")
    ...
    plt.plot(
        Time_window[0:T_plot],
        tau_in_dollar_per_ton_of_CO2_series[0:T_plot, 0],
        color="red",
        linestyle="-",
        label="Laissez-faire",
    )
    plt.plot(
        Time_window[0:T_plot],
        tau_in_dollar_per_ton_of_CO2_series[0:T_plot, 1],
        color="green",
        linestyle="-",
        label="Optimal",
    )
